...
class PlayerAuthPolicy {
  ...
  /// The current release uses the local email/password account on every target.
  /// Provider methods remain behind the gateway for a future authenticated
  /// backend, but are not advertised until they are actually connected.
  static Set<PlayerAuthMethod> methodsFor(PlayerAuthPlatform platform) {
    return const {PlayerAuthMethod.emailPassword};
  }
