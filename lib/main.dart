...
String _participantRoleLabel(
  ...
  final label = participant.kind == ParticipantKind.local ? 'Tú' : 'Rival';
...
class _GameScreenState extends State<GameScreen> {
  ...
  void _onGameChanged() {
    ...
    setState(() {});
    if (!engine.gameOver && !engine.currentPlayer.isHuman && !cpuThinking) {
      _playCpuTurn();
    }
  }
  ...
  Future<void> _playCpuTurn() async {
    ...
    if (!mounted || engine.gameOver || engine.currentPlayer.isHuman) {
      cpuThinking = false;
      return;
    }
  ...
  void _tapBoard(Offset localPosition, double boardSize) {
    if (!engine.currentPlayer.isHuman || engine.effectResolving) return;
      ...
      body: ColoredBox(
        ...
        child: SafeArea(
          child: Stack(
            fit: StackFit.expand,
            children: [
              Positioned.fill(child: _AnimatedThemeBackdrop(themeId: themeId)),
              LayoutBuilder(
                builder: (context, box) {
                  final sideBySide =
                      box.maxWidth >= 560 &&
                  ...
                  final spectatorBar = _buildSpectatorBar(compact: sideBySide);
              ...
              child: LayoutBuilder(
                builder: (context, box) => _AutoFitSingleLineText(
                  compact || box.maxWidth < 92
                      ...
                      : 'Parchese Pop · ${chaos ? 'Caos' : 'Tradicional'}',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    color: PopColors.navy,
...
class _GameSideRailState extends State<_GameSideRail> {
  ...
  void didUpdateWidget(covariant _GameSideRail oldWidget) {
    ...
    if (lastPlayerIndex != widget.engine.currentPlayerIndex) {
      ...
      selectedTab = widget.engine.currentPlayer.isHuman ? 0 : 1;
    }
  }
...
class _ParcheseBoardPainter extends CustomPainter {
  ...
  void _base(
    ...
    _baseThemeMotif(canvas, baseRect, theme, cell, progress);
    final nestCenter = Offset((col + 3.5) * cell, (row + 3.5) * cell);
    canvas.drawCircle(
      nestCenter + Offset(0, cell * .16),
      cell * 2.38,
      Paint()
      ...
      Paint()
        ..shader =
            RadialGradient(
              ...
              Rect.fromCircle(center: nestCenter, radius: cell * 2.35),
            ),
    );
    for (final point in const [
      ...
      final center = Offset((col + point.dx) * cell, (row + point.dy) * cell);
      canvas.drawCircle(
        center + Offset(0, cell * .09),
        cell * .50,
      ...
      canvas.drawCircle(
        ...
        Paint()
          ...
          ).createShader(Rect.fromCircle(center: center, radius: cell * .48)),
      );
      canvas.drawCircle(
        center,
        cell * .48,
    ...
    _baseLabel(canvas, cell, col, row, fullLabel);
  }
  ...
  void _baseLabel(Canvas canvas, double cell, int col, int row, String label) {
    ...
    final maxWidth = cell * 6.2;
    var text = buildPainter(cell * .70);
    if (text.width > maxWidth) {
      text = buildPainter(cell * .70 * maxWidth / text.width);
    }
    final center = Offset(
      (col + 3.5) * cell,
      (row == 0 ? row + .70 : row + 6.30) * cell,
    );
    canvas.save();
    canvas.translate(center.dx, center.dy);
  ...
  void _tokens(Canvas canvas, double cell) {
    ...
    for (final player in engine.players) {
      ...
      for (final token in player.tokens) {
        ...
        if (tokenStyle.motif != TokenMotif.star) {
          canvas.drawCircle(
            ...
            Paint()
              ...
              ..maskFilter = MaskFilter.blur(BlurStyle.normal, cell * .13),
          );
        }
        final radius = cell * .43;
        ...
        _tokenMark(canvas, center, cell, color, tokenStyle);
        if (canUseTwenty) {
          final badgeCenter = center + Offset(cell * .34, -cell * .34);
          canvas.drawCircle(
            badgeCenter,
            cell * .235,
...
class _GameVersionLabel extends StatefulWidget {
  const _GameVersionLabel();

  @override
  State<_GameVersionLabel> createState() => _GameVersionLabelState();
}

class _GameVersionLabelState extends State<_GameVersionLabel> {
  String label = 'v…';
  ...
  Future<void> _loadVersion() async {
    try {
      ...
      final resolved = buildNumber.isEmpty
          ? 'v${package.version}'
          : 'v${package.version}+$buildNumber';
      if (mounted) setState(() => label = resolved);
    } catch (_) {
      if (mounted) setState(() => label = 'v—');
    }
  }

  @override
  Widget build(BuildContext context) => Tooltip(
    message: '${appTranslate(context, 'Versión del juego')}: $label',
    child: Semantics(
      label: '${appTranslate(context, 'Versión del juego')}: $label',
      child: Container(
        key: const ValueKey('game-version-label'),
        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
        decoration: BoxDecoration(
          color: const Color(0xFFE7ECF7),
          borderRadius: BorderRadius.circular(9),
          border: Border.all(color: const Color(0xFFCDD6E8)),
        ),
        child: Text(
          label,
          maxLines: 1,
