...
void main() {
  test('iOS offers email, Google, and Apple', () {
    expect(PlayerAuthPolicy.methodsFor(PlayerAuthPlatform.ios), {
  ...
  test('Android offers email and Google but never Apple', () {
    expect(PlayerAuthPolicy.methodsFor(PlayerAuthPlatform.android), {
      PlayerAuthMethod.emailPassword,
      PlayerAuthMethod.google,
    });
    expect(
      PlayerAuthPolicy.allows(
        PlayerAuthPlatform.android,
        PlayerAuthMethod.apple,
      ),
      isFalse,
    );
  ...
  test(
    'desktop fallback offers email without unsupported provider buttons',
    () {
      for (final platform in [
        PlayerAuthPlatform.macos,
        PlayerAuthPlatform.windows,
        PlayerAuthPlatform.linux,
        PlayerAuthPlatform.web,
        PlayerAuthPlatform.unknown,
      ]) {
